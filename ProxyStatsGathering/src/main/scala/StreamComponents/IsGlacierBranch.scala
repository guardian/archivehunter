...
class IsGlacierBranch @Inject() (s3ClientMgr: S3ClientManager) extends GraphStage[UniformFanOutShape[ArchiveEntry,ArchiveEntry]]{
  ...
  override def createLogic(inheritedAttributes: Attributes): GraphStageLogic = new GraphStageLogic(shape) {
    val s3Client = s3ClientMgr.getClient()
