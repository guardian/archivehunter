...
class IngestProxyQueueSpec extends Specification with Mockito with ZonedDateTimeEncoder with StorageClassEncoder {
  ...
  "IngestProxyQueue!HandleDomainMessage" should {
    "dispatch CheckRegisteredThumb and CheckRegisteredProxy for each message, then dispatch HandleNextSqsMessage again" in new AkkaTestkitSpecs2Support {
      ...
      val fakeEntry1 = ArchiveEntry("fake-id-1", "fakebucket", "/path/to/file", Some("region"), Some(".ext"), None, 1234L, ZonedDateTime.now().withFixedOffsetZone(), "etag", MimeType("video", "mp4"), false, StorageClass.STANDARD_IA, Seq(), false, None)
      val fakeEntry2 = ArchiveEntry("fake-id-2", "fakebucket", "/path/to/file2", Some("region"),Some(".ext"), None, 1234L, ZonedDateTime.now().withFixedOffsetZone(), "etag2", MimeType("video", "mp4"), false, StorageClass.STANDARD_IA, Seq(), false, None)
