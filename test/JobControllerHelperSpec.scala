...
class JobControllerHelperSpec extends Specification with Mockito with MockitoMatchers {
  ...
  "JobControllerHelper.thumbnailOrOriginalMedia" should {
    ...
    "perform an index lookup if passed media type" in new AkkaTestkitSpecs2Support{
      ...
      val mockResponse = ArchiveEntry("fake-id","fake-bucket","fake-path",Some("region"),None,1234L,ZonedDateTime.now(),"fake-etag",MimeType("video","something"),false,StorageClass.STANDARD, Seq(), false, None)
  ...
  "JobControllerHelper.updateProxyRef" should {
    "create a ProxyLocation object based on the input parameters" in new AkkaTestkitSpecs2Support {
      ...
      val sourceEntry = ArchiveEntry("fake-id","fake-bucket","fake-path",Some("region"),None,1234L,ZonedDateTime.now(),"fake-etag",MimeType("video","something"),false,StorageClass.STANDARD, Seq(),false, None)
