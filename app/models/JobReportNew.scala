...
case class JobReportNew(status:String, log:Option[String], jobId:String, input:Option[String], output:Option[String]) {
  /**
    * performs base64 decoding on the log field and returns a new instance with the decoded content
    * @return
    */
//  def decodeLog:Option[Try[JobReportNew]] = {
//    log.map(actualLog=> {
//      val dec = Base64.getDecoder
//      Try {
//        this.copy(log = Some(dec.decode(actualLog).map(_.toChar).mkString))
//      }
//    })
//  }
  /**
